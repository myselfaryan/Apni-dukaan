...
router.post('/get-user-profile', upload.none(), async (req, res) => {
    ...
    if (user_auth) {
        let user_details = await UserDetails.findOne({ personal_contact_number: user_auth.personal_contact_number });
        console.log("User details: " + user_details);
        let business_info = await BusinessInfo.findOne({ personal_contact_number: user_auth.personal_contact_number });
        console.log("business info: " + business_info);
        if (user_details) {
            console.log(user_details);
            return res.status(200).json({ user_details: user_details, business_info: business_info, status: 'ok' });
        } else {
...
router.post('/profile-image', upload.none(), async (req, res) => {
    ...
    try {
        ...
        const imagePath = path.join(__dirname, '../../public/uploads/profile_images/', user_details.profile_image_id);
